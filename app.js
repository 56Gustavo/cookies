...
app.get('/adicionar/:id', (req, res) => {
    ...
    if (produto) {
        ...
        produto.horarioAdicao = `${dataHora.toLocaleDateString('pt-BR')} ${dataHora.toLocaleTimeString('pt-BR')}`;
...
app.get('/carrinho', (req, res) => {
    ...
    res.send(`
        ...
        <ul>
            ${carrinho.map(
                (produto) => `<li>${produto.nome} - ${produto.preco} ${produto.tipo} (Adicionado em: ${produto.horarioAdicao})</li>`
            ).join('')}
        </ul>

        <h2>Produtos do Mesmo Tipo do Último Adicionado:</h2>
        <ul>
            ${produtos.filter((produto) => produto.tipo === ultimoTipoAdicionado).map(
